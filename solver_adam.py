...
def get_exact_derivatives(x1, x2):
    ...
    # u derivatives (Intermediate calculations based on pure sin^2)
    sin_pix = torch.sin(pi * x1)
    cos_pix = torch.cos(pi * x1)
    sin_piy = torch.sin(pi * x2)
    cos_piy = torch.cos(pi * x2)

    # Raw derivatives of sin^2(pix)sin^2(piy) without factor
    ...
    # Apply factor to U and all its derivatives
    u_exact = u_factor * raw_u
